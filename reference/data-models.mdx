...
interface ItemRefund {
  amount: number;           // returned to the buyer
  penalty: number;          // withheld on buyer fault, else 0
  buyerFault: boolean;      // true when declined as the buyer's fault
}
...
interface MarketListing {
  ...
  inspectUrl?: string;
  assetId?: string;          // Steam asset id — catalog/listing reads only, not on trades/webhooks
  delivery?: string;
}
