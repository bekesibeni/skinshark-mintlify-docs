...
// Set on a trade item's `error` when its `status` is `failed`.
...
```ts
interface TradeItem {
  id: string;               // stable handle; pass as itemId when cancelling
  externalId?: string;      // your own per-item ref, when supplied at buy time
  name: string | null;
  ...
  status: TradeStatus;
  tradable: boolean;
  error?: TradeFailureCode; // set when status is "failed"
  errorDetail?: string;     // raw marketplace reason, debug only — opaque
  refund?: ItemRefund;      // present only on refunded buy items; amount + penalty === price
}

// Present only on refunded items (failed/canceled/declined/reverted). Buy trades only.
interface ItemRefund {
  amount: number;           // returned to the buyer; 0 when the user reverted the trade
  penalty: number;          // withheld on buyer fault, else 0; the whole price on a user revert
  buyerFault: boolean;      // true when declined as the buyer's fault or reverted by the user
}
...
interface BuyItem {
  ...
  externalId?: string;         // your own per-item ref; distinct per request, accepted by cancel
}
...
interface QuickBuyBody {
  ...
  phase?: DopplerPhase;        // Doppler / Gamma Doppler only; combines with either delivery mode
  tradeUrl?: string;
  externalId?: string;
  externalIds?: string[];      // one ref per unit, in order; exactly `amount` entries, distinct
}
...
// POST /market/transactions/{tradeId}/cancel — best-effort over every cancellable item.
interface CancelTradeResponse {
  tradeId: string;
  requested: number;        // items still cancellable, and therefore attempted
  cancelled: number;
  failed: number;
  items: Array<{
    itemId: string;
    externalId?: string;
    status: "cancelled" | "failed";
    reason?: string;        // stable error key when refused, e.g. "TRADE_CANCEL_TOO_SOON"
  }>;
}
```
...
See the [Selling guide](/guides/selling) for the full flow.

```ts
// GET /market/sell/prices — the payout book ("what we pay")
interface SellPrice {
  marketHashName: string;
  deposit: number;             // what we pay, USD
  accepted: boolean;           // currently buyable by our bots
}
type SellPricesResponse = SellPrice[];

// GET /market/sell/inventory — the user's items priced for selling
interface SellInventoryItem {
  id: string;                  // encoded item id — pass to POST /market/sell items[].id
  assetid: string;             // raw Steam asset id
  name: string;
  marketHashName: string;
  type: string;
  iconUrl: string;             // raw Steam icon hash
  tradable: boolean;
  marketPrice?: number;        // platform reference price, USD
  price: number;               // our offer, USD — send this exact value
  accepted: boolean;           // quote exists AND tradable; only accepted items can be sold
  exterior?: string;
  rarity?: string;
  color?: string;
  previewToken?: string;
...
interface SellItem {
  // Provide exactly one of id or assetid.
  id?: string;                 // from SellInventoryItem.id
  assetid?: string;            // raw Steam asset id
  price: string;               // decimal string — must equal the live quote to the cent
}

interface SellBody {
  items: SellItem[];           // 1–100
  tradeUrl?: string;           // must belong to the user's linked Steam account
  externalId?: string;         // your correlation id; replays return the same Trade
}

interface SellResponse {
  id: string;
  status: TradeStatus;         // "initiated" right after submit
  itemCount: number;
  totalPrice: number;          // payout in wallet currency, at a locked FX rate
  currency: Currency;
  createdAt: string;
}
```
...
interface SearchResultItem {
  ...
  // floor, null when none. Omitted when no market price.
...
interface MarketListing {
  id: string;
  assetId: string | null;    // Steam asset id — catalog/listing reads only, not on trades/webhooks
  classId: string | null;
  ...
  collection?: string;
  color?: string;
  wear?: string | null;      // float value as a string
  ...
  // bare hex inspect token; rebuild as
  // steam://run/730//+csgo_econ_action_preview%20{previewToken}
