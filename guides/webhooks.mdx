...
The key bit: `express.raw({ type: "application/json" })` so the body is a
`Buffer`, not pre-parsed JSON. Verify, then parse.
...
app.post(
  ...
  async (req, res) => {
    const result = verifySkinSharkSignature(
    ...
    const event = JSON.parse((req.body as Buffer).toString("utf8")) as SkinSharkEvent;
    const eventId = req.header("webhook-id")!;

    // Idempotency: persist before acking; redeliveries reuse webhook-id.
...
type SkinSharkEvent = {
  id: string;
  type: string;            // "trade.completed", "deposit.failed", ...
  createdAt: string;
  data: { trade?: SerializedTrade; item?: SerializedItem; deposit?: SerializedDeposit; settlement?: { houseFeeCents: number; merchantFeeCents: number } };
};
```
