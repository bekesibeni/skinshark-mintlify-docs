...
|---|---|---|
| `LISTING_UNAVAILABLE` | The listing sold or was delisted. | Retry with another listing. |
| `NO_LISTING_AT_PRICE` | Nothing on sale at or below your `maxPrice`. | Re-quote higher, or wait for restock. |
...
| `BUYER_TRADE_RESTRICTED` | The marketplace suspended this buyer, e.g. for repeated trade-offer no-shows — usually until a stated time. | User accepts their pending offers; retrying sooner cannot succeed. |
...
(e.g. `"market:ITEM_SOLD item already sold"`) for debugging only — treat it as
